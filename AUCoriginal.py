...
from sklearn.preprocessing import StandardScaler, OneHotEncoder  # To standardize the features and encode categorical variables
from sklearn.compose import ColumnTransformer
from sklearn.pipeline import Pipeline
from sklearn.impute import SimpleImputer  # To hthe Multi-Layer Perceptron model
from deap import base, creator, tools, algorithms  # To implement thandle missing values
from sklearn.neural_network import MLPClassifier  # To implement e Genetic Algorithm
...
y_probs = final_model.predict_proba(X_test)[:, 1]  # Probability estimates for the positive class
final_auc_roc = roc_auc_score(y_test, y_probs)  # Compute AUC-ROC
print(f"Final model AUC-ROC: {final_auc_roc:.4f}")  # Print the AUC-ROC score
...
# After model training, calculate and print AUC-ROC for the model
y_probs = final_model.predict_proba(X_test)[:, 1]  # Probability estimates for the positive class
final_auc_roc = roc_auc_score(y_test, y_probs)  # AUC-ROC calculation
print(f"Final model AUC-ROC: {final_auc_roc:.4f}")  # Print AUC-ROC score
